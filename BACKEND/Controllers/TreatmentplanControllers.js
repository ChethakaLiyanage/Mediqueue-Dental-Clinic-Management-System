...
/* ----------------------- NEW: Patient-specific endpoint ----------------------- */
const getMyTreatments = async (req, res) => {
  try {
    // Get patient code from authenticated user's data
    const patientCode = req.user?.patientCode || req.user?.patient?.patientCode;
    
    console.log("getMyTreatments - User:", req.user?.id, "PatientCode:", patientCode);
    
    if (!patientCode) {
      console.warn("No patient code found for user:", req.user?.id);
      return res.status(400).json({ 
        message: "Patient code not found for current user",
        userId: req.user?.id,
    ...
    const filter = includeArchived 
      ? { patientCode } 
      : { patientCode, isDeleted: false };

    console.log("getMyTreatments - Filter:", filter);

    const treatments = await Treatmentplan.find(filter)
      .sort({ created_date: -1 }) // Most recent first
      .lean();

    console.log("getMyTreatments - Found treatments:", treatments.length);
...
module.exports = {
  ...
  getMyTreatments, // NEW: Added this export
};